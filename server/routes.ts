...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/recipes/recommend", aiRecipeRateLimit, async (req, res, next) => {
    ...
    try {
      ...
      const result = await recommendRecipes(ingredients, { signal: abortController.signal });
      
      const mappedRecipes = result.recipes.map(recipe => {
        const fullRecipe = {
          ...recipe,
          slug: recipe.id,
          image: "",
        ...
        if (!recipesDatabase.some(r => r.id === fullRecipe.id)) {
          recipesDatabase.push(fullRecipe as any);
        }
  ...
  app.post("/api/recipe/:id/progress", externalApiRateLimit, requireAuth, (req, res) => {
    try {
      const { id } = req.params;
